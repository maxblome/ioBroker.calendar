...
class Google {
    constructor(clientId, clientSecret, fqdn, port) {
        ...
        this.oauth2 = this.getAuthentication();
    }

    async getAuthenticationTokens(code) {
    ...
    getAuthentication() {
        if(this.clientId && this.clientSecret && this.fqdn && this.port)  {
            return new google.auth.OAuth2(this.clientId, this.clientSecret, `http://${this.fqdn}:${this.port}/google`);
        } else {
            throw 'Client id, client secret, fqdn or port missing.';
        }
    }
