 ...
 * @returns {Promise<Object>} Object
 */
async function queryEvents(adapter, href, username, password, rangeStart, rangeEnd) {

    const auth = 'Basic ' + Buffer.from(username + ':' + password).toString('base64');
    let range = '';

    if(typeof rangeStart === 'object' && typeof rangeEnd === 'object') {
        range = `<c:time-range start="${rangeStart.year}${rangeStart.month}${rangeStart.date}T000000Z" end="${rangeEnd.year}${rangeEnd.month}${rangeEnd.date}T000000Z"></c:time-range>`;
    } else if(typeof rangeStart === 'object') {
        range = `<c:time-range start="${rangeStart.year}${rangeStart.month}${rangeStart.date}T000000Z"></c:time-range>`;
    }
            ...
            if(!error && (response.statusCode == 200 || response.statusCode == 207)) {
                ...
                parser.parseString(body, (error, result) => {
                    ...
                    if(error) {
                        ...
                        resolve(normalize(result).multistatus.response);
                    }
                });
